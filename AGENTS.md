...
1. **`docs/llm-wiki/`** — product rules for agents (i18n, Grok Build catalog).  
   ...
   - [session-continuity.md](docs/llm-wiki/session-continuity.md) — load/bootstrap; **pasted UUIDs default to Grok Workspace session ids** (not CLI agent ids)
   ...
   - [remote-im.md](docs/llm-wiki/remote-im.md) — **Remote IM** GUI 配置全渠道 · Bridge · Grok Build  
   ...
   - [office.md](docs/llm-wiki/office.md) — **Grok Office**: daily-work surface (papers / slides / sheets), **not** Grok Build’s coding IDE; not an editor suite
   - [studio.md](docs/llm-wiki/studio.md) — **Grok Studio**: Imagine image / video canvas; official-aux `image_gen` + `image_to_video`
   - [discord-presence.md](docs/llm-wiki/discord-presence.md) — **Discord Rich Presence**: project / workspace / plan / progress / elapsed; Settings → General → App
...
1b. **Release (AI handoff)** — **[docs/llm-wiki/release.md](docs/llm-wiki/release.md)** is the single source for ship steps. Platforms / local build: [docs/BUILD.md](docs/BUILD.md). Window chrome: `tauri.macos.conf.json` (Overlay) vs `tauri.windows.conf.json` (frameless).  
   ...
   - **Contributors**: every release refresh circular-avatar galleries via `python3 scripts/update-contributors.py` (`README.md` markers). No square table + contrib.rocks dual track.

1c. **Open-source surface** — public docs: `README.md`, `LICENSE`, `CONTRIBUTING.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md`, `CHANGELOG.md`. Do not commit secrets, `auth.json`, or local agent homes.

2. Do **not** hardcode user-facing copy in any language. Use `createT(locale)` / `t()`. Thirteen locales ship (`de` `en` `es` `fil` `fr` `id` `it` `ja` `ko` `pt-BR` `ru` `ta` `uk`); `en` is the key authority. Complete catalogs must stay in lockstep with `en`. `zh` / `zh-TW` folders stay on disk but are unshipped (OS Chinese → `en`). Never fork a date or number format on `locale === "zh"` — use `intlLocale()` / `isTightScript()`. See [i18n.md](docs/llm-wiki/i18n.md).
...
7. **App.tsx growth freeze** — do **not** add new `useState` / large feature blocks to `src/App.tsx`. New product state and UI must land in domain modules (`src/providers/`, `src/hooks/`, `src/components/`, `src/lib/`).
