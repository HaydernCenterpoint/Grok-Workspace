...
#/settings/appearance/interface     → 外观 · 界面（工作台 chrome / 聊天展示；`settings-anchor-workbenchChrome`）
...
3. **i18n**：先写 `en`，再锁步 13 个已上架目录；停用的 `zh` / `zh-TW` 可顺手补上。`appearance.workbenchChrome` 已登记（`settings-anchor-workbenchChrome`）。  
4. **禁止**只改 UI 不登记（搜索会漏）。  
