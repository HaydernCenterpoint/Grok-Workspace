...
import { CodexWorkbenchShell } from "@/components/CodexWorkbenchShell";
import { MainTopOverflow } from "@/components/MainTopOverflow";
import {
  applyWorkbenchChrome,
  isWorkbenchChrome,
  loadWorkbenchChrome,
  WORKBENCH_CHROME_CHANGE_EVENT,
  type WorkbenchChrome,
} from "@/lib/workbenchChrome";
import { useGrokOffice } from "@/hooks/useGrokOffice";
import { StudioComposerTools } from "@/components/StudioSurface";
import { useStudioComposer } from "@/hooks/useStudioComposer";
import { wrapStudioAgentText } from "@/lib/studio";
import {
...
export function AppWorkbench() {
  ...
  const [workbenchChrome, setWorkbenchChromeState] = useState<WorkbenchChrome>(
    () => loadWorkbenchChrome(),
  );
  useEffect(() => {
    applyWorkbenchChrome(loadWorkbenchChrome());
    const onChange = (event: Event) => {
      const next = (event as CustomEvent<WorkbenchChrome>).detail;
      if (isWorkbenchChrome(next)) setWorkbenchChromeState(next);
    };
    window.addEventListener(WORKBENCH_CHROME_CHANGE_EVENT, onChange);
    return () =>
      window.removeEventListener(WORKBENCH_CHROME_CHANGE_EVENT, onChange);
  }, []);
  ...
  const composerChoiceRef = useRef<SurfaceComposerChoice>({
    modelId,
    effort,
    mode,
    policy,
  });
  composerChoiceRef.current = { modelId, effort, mode, policy };
  ...
  const applySurfaceComposerChoice = useCallback(
    (choice: SurfaceComposerChoice) => {
      setModelId(choice.modelId);
      setEffort(choice.effort);
      setMode(choice.mode);
      setPolicy(choice.policy);
    },
    [],
  );

  const persistComposerPrefs = useCallback(
    (body: Parameters<typeof api.composerPrefsSet>[0]) => {
      if (!persistComposerPrefsToHost(workModeRef.current)) {
        return Promise.resolve(null);
      }
      return api.composerPrefsSet(body);
    },
    [],
  );
  ...
  const applyComposerPrefs = useCallback(
    (prefs: api.ComposerPrefs, catalog: ModelOption[]) => {
      if (!persistComposerPrefsToHost(workModeRef.current)) {
        applySurfaceComposerChoice(
          resolveSurfaceComposerChoice(workModeRef.current),
        );
        return;
      }
  ...
  const openOfficeFilesPane = useCallback(() => {
    setSideWorkbench((s) => {
      const next = openSideTabFromPicker(s, "file", {
        isGitProject: sideIsGitProject,
        workMode: "office",
      });
  ...
  const enterOffice = useCallback(() => {
    ...
    setShowUserMenu(false);
    openOfficeFilesPane();
    bottomTerminal.closePanel();
    void ensureProjectOfficeSkill(effectiveProjectPath);
    if (typeof window !== "undefined" && window.location.hash !== OFFICE_HASH) {
      window.location.hash = OFFICE_HASH;
    }
  }, [setWorkMode, openOfficeFilesPane, bottomTerminal, effectiveProjectPath]);

  const exitOffice = useCallback(() => {
    setWorkMode("code");
    if (
      typeof window !== "undefined" &&
      (isOfficeHash(window.location.hash) || isStudioHash(window.location.hash))
    ) {
  ...
  const enterStudio = useCallback(() => {
    ...
    closeAsidePane();
    bottomTerminal.closePanel();
    if (typeof window !== "undefined" && window.location.hash !== STUDIO_HASH) {
  ...
  useEffect(() => {
    ...
    setSideWorkbench((s) => filterSideTabsForWorkMode(s, "office"));
    bottomTerminal.closePanel();
  }, [workMode, bottomTerminal.closePanel]);

  useEffect(() => {
    if (workMode !== "studio") return;
    closeAsidePane();
    bottomTerminal.closePanel();
  }, [workMode, closeAsidePane, bottomTerminal.closePanel]);

  useEffect(() => {
    const prev = workModeRef.current;
    if (prev === workMode) return;
    saveSurfaceComposerChoice(prev, composerChoiceRef.current);
    workModeRef.current = workMode;
    if (persistComposerPrefsToHost(workMode)) {
      const cached = resolveSurfaceComposerChoice("code");
      applySurfaceComposerChoice(cached);
      void api
        .composerPrefsResolve({
        ...
        .then((prefs) => {
          if (workModeRef.current !== "code") return;
          applyComposerPrefs(prefs, availableModelsRef.current);
        })
        .catch(() => {
          /* keep cached Build choice */
        });
      return;
    }
    applySurfaceComposerChoice(resolveSurfaceComposerChoice(workMode));
  }, [applyComposerPrefs, applySurfaceComposerChoice, workMode]);

  useEffect(() => {
    if (persistComposerPrefsToHost(workMode)) return;
    saveSurfaceComposerChoice(workMode, {
      modelId,
      effort,
      mode,
      policy,
    });
  }, [workMode, modelId, effort, mode, policy]);

  useEffect(() => {
    if (workMode !== "code" && mainPane === "kanban") {
      setMainPane("chat");
    }
  }, [workMode, mainPane]);
  ...
  // Hash route: #/settings[/section[/tab]][?pr=N] | #/automations | #/kanban | #/office | #/studio | #/workbench
  ...
  useEffect(() => {
    const syncFromHash = () => {
      ...
      if (raw.startsWith("settings")) {
        ...
        setAppView("workbench");
        setMainPane("kanban");
      } else if (isOfficeHash(fullHash)) {
        setAppView("workbench");
        setMainPane("chat");
        setWorkMode("office");
      } else if (isStudioHash(fullHash)) {
        setAppView("workbench");
        setMainPane("chat");
        setWorkMode("studio");
  ...
  const handoffBuildProjectToOffice = async () => {
    ...
    enterOffice();
    const named = proj && !isGeneralProject(proj);
    const seed = named
      ? tr("composer.officeReportDraft", {
  ...
  const executeSend = async (opts: {
    ...
    if (workMode === "studio" && !opts.agentTextOverride?.trim()) {
      agentText = wrapStudioAgentText(agentText, {
        kind: studio.kind,
  ...
  const openSideSkillsPanel = useCallback(() => {
    if (!surface.composerSkills) return;
    setShowComposerPlus(false);
    setSlashQuery(null);
  ...
  }, [activeProject?.id, persistComposerPrefs, session.sessionId, showToast]);
  ...
  const runForkSession = useCallback(
    async (
      ...
      try {
        ...
        // bindProject wins when we just added a worktree (list may still be stale).
        const openProj =
          bindProject ??
  ...
  const applyPermissionPolicy = useCallback(
    (next: PermissionPolicyId) => {
      ...
      const commit = () => {
        setPolicy(next);
        if (!persistComposerPrefsToHost(workModeRef.current)) return;
        void api
      ...
      if (item.kind === "action") {
        switch (item.action) {
          ...
          case "studio":
            ...
            return;
          case "report":
            void handoffBuildProjectToOffice();
            return;
          case "live-voice":
          case "liveVoice":
            if (!voiceGate.available) {
  ...
  shortcutHandlersRef.current = {
    ...
    toggleBottomTerminal: () => {
      if (!surface.bottomTerminal) return;
  ...
  const resendLastUserTurn = useCallback(
    async (
      ...
      try {
        ...
        if (switchModel && persistComposerPrefsToHost(workModeRef.current)) {
          if (api.isTauri()) {
            try {
    ...
    >
      ...
      {appGate === "loading" && (
        <div className="setup-gate" data-testid="setup-booting">
          <div
            ...
            {...titlebarMax}
          />
          <div className="setup-gate__center setup-gate__center--boot">
            <GrokLogoLoader
              size={64}
              active={!bootDetectTimedOut}
              label={
                bootDetectTimedOut
            ...
            {bootDetectTimedOut ? (
              <>
                <h1 className="setup-title">{tr("setup.detectTimeout")}</h1>
                <p className="setup-subtitle">{tr("setup.detectTimeoutHint")}</p>
      ...
      {settingsPresence.mounted ? (
        ...
        >
          ...
          onDiscordPresence={(v) => {
          saveDiscordPresencePref(v, localStorage);
            ...
            <div className="sidebar-brand-row__left">
              <SidebarProductSwitch
                ...
                labels={{
                  ...
                  office: tr("sidebar.office"),
                  studio: tr("sidebar.studio"),
                  buildTip: tr("sidebar.buildTip"),
                  officeTip: tr("sidebar.officeTip"),
                  studioTip: tr("sidebar.studioTip"),
                  switcher: tr("sidebar.switchProduct"),
                }}
        ...
        >
                ...
                overflowItems={[
                  ...(surface.bottomTerminal
                    ? [
                        {
                          id: "terminal",
                          label: bottomTerminal.state.open
                            ? tr("terminal.toggleHide")
                            : tr("terminal.toggleShow"),
                          onClick: () => bottomTerminal.toggle(),
                        },
                      ]
                    : []),
                  {
                    id: "aside",
                    label: layout.asideCollapsed
          ...
          <ConversationThreadLive
            ...
            officeProjectName={
              isOffice &&
              activeProject &&
              !isGeneralProject(activeProject)
                ? projectDisplayName(activeProject, tr)
                : null
            }
            onOfficeStart={(kind) => {
          ...
          <ComposerDock
            ...
            className={resizingSidebar ? "is-sidebar-resizing" : undefined}
            style={
              sideDockActive
                ? ({
          ...
          >
            ...
            {(() => {
              ...
              const showComposerProjectRow =
                !phoneLayout && surface.composerProject;
            ...
            >
              ...
              >
                ...
                {surface.composerSkills && !phoneLayout ? (
                ...
                {!phoneLayout ? (
                  <>
                    {goalMode && surface.composerAccess ? (
          ...
          {surface.bottomTerminal && bottomTerminalMounted ? (
            <Suspense fallback={null}>
              <BottomTerminal
                locale={locale}
                projectPath={effectiveProjectPath}
      ...
      {phoneLayout ? (
        <>
          <PhoneComposerToolsSheet
            ...
            showProject={surface.composerProject}
            showModel={surface.composerModel}
            showAccess={surface.composerAccess}
            showContext={surface.composerContext}
          />
          <PhoneAccountSheet
