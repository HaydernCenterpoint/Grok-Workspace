...
import { zh as parkedZh } from "./messages/zh";
...
describe("i18n catalog", () => {
  ...
  it("interpolates variables", () => {
    ...
    expect(t("ja", "project.trustFirst", { name: "デモ" })).toContain("デモ");
  ...
  it("createT binds locale (English is the product default)", () => {
    ...
    const ja = createT("ja");
    expect(ja("sidebar.settings")).not.toBe("Settings");
    const ru = createT("ru");
  ...
  it("non-Chinese catalogs do not copy Simplified Chinese values", () => {
    ...
    for (const loc of LOCALES) {
      ...
      for (const [k, v] of Object.entries(messages[loc])) {
        ...
        if (v === parkedZh[key] && simplifiedOnly.test(v)) {
          leaked.push(`${loc}.${key}`);
        }
      }
    }
...
describe("resolveLocale", () => {
  ...
  it("unships Chinese tags to English", () => {
    expect(resolveLocale("zh")).toBe("en");
    expect(resolveLocale("zh-TW")).toBe("en");
    expect(resolveLocale("zh-tw")).toBe("en");
    expect(resolveLocale("zh_TW")).toBe("en");
  ...
  it("accepts case/alias variants of Russian and English", () => {
...
describe("resolveLocaleFromSystem", () => {
  ...
  it("maps Chinese tags to English while catalogs are unshipped", () => {
    expect(resolveLocaleFromSystem("zh")).toBe("en");
    expect(resolveLocaleFromSystem("zh-CN")).toBe("en");
    expect(resolveLocaleFromSystem("zh_CN")).toBe("en");
    expect(resolveLocaleFromSystem("zh-Hans")).toBe("en");
    expect(resolveLocaleFromSystem("zh-Hans-CN")).toBe("en");
    expect(resolveLocaleFromSystem("zh-SG")).toBe("en");
    expect(resolveLocaleFromSystem("zh_CN.UTF-8")).toBe("en");
    expect(resolveLocaleFromSystem("zh-TW")).toBe("en");
    expect(resolveLocaleFromSystem("zh_TW")).toBe("en");
    expect(resolveLocaleFromSystem("zh-Hant")).toBe("en");
    expect(resolveLocaleFromSystem("zh-Hant-TW")).toBe("en");
    expect(resolveLocaleFromSystem("zh-HK")).toBe("en");
    expect(resolveLocaleFromSystem("zh-MO")).toBe("en");
    expect(resolveLocaleFromSystem("zh_TW.UTF-8")).toBe("en");
  });
...
describe("intlLocale / htmlLangForLocale", () => {
  ...
  it("routes catalogs with thin ICU data to a data-complete tag", () => {
    expect(intlLocale("zh")).toBe("en");
    expect(intlLocale("zh-TW")).toBe("en");
...
describe("parseLocalePreference / resolveLocalePreference", () => {
  it("keeps system and canonical locales", () => {
    ...
    expect(parseLocalePreference("zh")).toBe("en");
    expect(parseLocalePreference("zh-TW")).toBe("en");
  });

  it("normalizes aliases and invalid values", () => {
    expect(parseLocalePreference("zh-cn")).toBe("en");
    expect(parseLocalePreference("zh-hant")).toBe("en");
  ...
  it("resolves system preference via an explicit lang tag", () => {
    expect(resolveLocalePreference("system", "zh-CN")).toBe("en");
    expect(resolveLocalePreference("system", "zh-TW")).toBe("en");
  ...
  it("returns explicit preferences unchanged", () => {
    expect(resolveLocalePreference("ja", "en-US")).toBe("ja");
