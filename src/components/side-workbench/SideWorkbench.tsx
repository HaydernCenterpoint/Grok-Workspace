...
export function SideWorkbench({
  ...
  const pick = useCallback(
    (kind: SidePickerKind) => {
      const next = openSideTabFromPicker(state, kind, { isGitProject, workMode });
      if ("created" in next) {
        setState(next);
      }
    },
    [state, isGitProject, workMode, setState],
  );
  ...
  useEffect(() => {
    ...
    if (openRequest.type === "file" && openRequest.path) {
      ...
      openRequest.type === "changes" &&
      isGitProject &&
      workSurfaceChrome(workMode).sideReview
    ) {
