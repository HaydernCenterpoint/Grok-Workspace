...
import { canCaptureOfficeKind, captureOfficeFile } from "@/lib/officeDoc";
...
export interface OfficeDocumentPreviewProps {
   ...
   */
  embedded?: boolean;
  /** Trusted project root — enables capture to `*.office.json`. */
...
export function OfficeDocumentPreview({
  ...
  const onCapture = async () => {
    ...
    setCaptureNote(null);
    try {
      const dest = await captureOfficeFile({
        projectPath: root,
        relativePath: rel,
        name,
        kind,
        buffer: load.status === "ready" ? load.buffer : null,
        text: textFallback,
      });
      const saved = dest.split("/").pop() ?? dest;
      setCaptureNote(tr("office.doc.captureOk", { name: saved }));
    } catch (e) {
      setCaptureNote(tr("office.doc.exportFail", { name: String(e ?? name) }));
    } finally {
      setCapturing(false);
    }
  };
      ...
      <div className="office-preview office-preview--center">
        ...
        <div className="office-preview__actions">
          ...
          {canCapture ? (
            ...
            >
              {capturing ? tr("resources.saving") : tr("office.doc.capture")}
            </button>
          ) : null}
        </div>
        {captureNote ? (
          <div className="office-preview__sub" role="status">
            {captureNote}
          </div>
        ) : null}
      </div>
      ...
      >
        ...
        {embedded && canCapture ? (
          <div className="office-preview__bar office-preview__bar--controls">
            ...
            {captureNote ? (
              <span className="office-preview__bar-title" role="status">
                {captureNote}
              </span>
            ) : null}
          </div>
