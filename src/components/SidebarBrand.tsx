...
export const SidebarBrand = memo(function SidebarBrand({
  ...
  // Preference off / no known brand: Grok mark + product label (Build / Office).
  // Never use the active provider name here — that would look like a partial swap.
  return (
    <>
      <GrokLogo size={20} />
      <span className="sidebar-brand-row__label">{label}</span>
    </>
  );
});
