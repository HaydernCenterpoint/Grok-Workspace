...
export function GeneralSection() {
  ...
  const [presenceClientId, setPresenceClientId] = useState(loadDiscordClientId);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void api.discordPresenceProbe(presenceClientId || undefined).then((r) => {
      if (cancelled) return;
      setPresenceError(r.ok ? null : r.error);
    });
    return () => {
      cancelled = true;
    };
  }, [discordPresence, presenceClientId]);
            ...
            <div className="settings-card">
              ...
              {onDiscordPresence ? (
                <div
                  className={
                    "settings-row settings-row--stack" +
                    rowHighlight("settings-anchor-discordPresence")
                  }
                  id="settings-anchor-discordPresence"
                >
                  <div className="settings-tray-notify__row-main">
                    <div className="settings-row__text">
                      <div className="settings-row__label">
                      ...
                      <div className="settings-row__desc">
                        {t("settings.discordPresenceDesc")}
                      </div>
                    </div>
                    <UiCheck
                      checked={!!discordPresence}
                      onChange={() => onDiscordPresence(!discordPresence)}
                      ariaLabel={t("settings.discordPresence")}
                    />
                  </div>
                  {discordPresence ? (
                    <>
                      <input
                        ...
                        spellCheck={false}
                        placeholder={t(
                          "settings.discordPresence.clientIdPlaceholder",
                        )}
                        value={presenceClientId}
                        aria-label={t("settings.discordPresence.clientId")}
                        onChange={(e) =>
                          setPresenceClientId(
                        ...
                        onBlur={() => {
                          const id = saveDiscordClientId(presenceClientId);
                          setPresenceClientId(id);
                        }}
                      />
                      <div className="settings-row__desc">
                        {t("settings.discordPresence.clientIdDesc")}
                      </div>
                    </>
                  ) : null}
                  <div
                    className={
                      "settings-tray-notify__status" +
                      (!discordPresence || !presenceError ? " is-info" : "")
                    }
                    role="status"
                  >
                    {t(
