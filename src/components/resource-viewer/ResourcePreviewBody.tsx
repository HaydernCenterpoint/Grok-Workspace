...
import { isOfficeDocPath } from "@/lib/officeDoc";
...
const OfficeArtifactPreview = lazy(async () => {
  const m = await import("@/components/OfficeArtifactPreview");
  return { default: m.OfficeArtifactPreview };
});
...
export function ResourcePreviewBody({
          ...
          <OverlayScroll className="rp-editor__preview-scroll">
            <Suspense fallback={null}>
              <OfficeArtifactPreview
                ...
                relativePath={activeTab.relativePath || preview.relativePath}
              />
            </Suspense>
          </OverlayScroll>
        ) : (
  ...
  if (
    ...
    return (
        ...
        embedded
        projectPath={projectPath}
        relativePath={preview.relativePath || activeTab?.relativePath}
        />      </Suspense>
    );
  }
  ...
  switch (preview.kind) {
    ...
    case "json": {
      const officeText = activeTab?.draftText ?? preview.text ?? "";
      if (isOfficeDocPath(preview.name) && officeText) {
        return (
          <OverlayScroll className="rp-editor__preview-scroll">
            <Suspense fallback={null}>
              <OfficeArtifactPreview
