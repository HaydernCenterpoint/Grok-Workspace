 ...
 *
 * - `comfortable` (default when chrome is Classic)
 * - `compact`: tighter item / bubble / list gaps (unset + Codex chrome)
 */
...
export function loadChatDensity(
  ...
  try {
    const raw = storage.getItem(CHAT_DENSITY_STORAGE_KEY);
    if (raw == null && loadWorkbenchChrome(storage) === "codex") {
      return "compact";
    }
    return parseChatDensity(raw);
  } catch {
    return DEFAULT_CHAT_DENSITY;
  }
}
