...
describe("buildErrorDeck", () => {
  ...
  it("returns localized copy for ja", () => {
    const cli = buildErrorDeck("CLI_NOT_FOUND", "ja");
    expect(cli.problem).not.toBe(buildErrorDeck("CLI_NOT_FOUND", "en").problem);
  ...
  it("classifies official free-usage-exhausted as QUOTA even when host said NETWORK_PROVIDER", () => {
    ...
    const ja = buildErrorDeck("QUOTA_EXCEEDED", "ja");
    expect(ja.problem).not.toBe(deck.problem);
  });
  ...
  it("keeps a bare 429 as Rate limited, not free-usage copy", () => {
    ...
    const ja = buildErrorDeck("RATE_LIMITED", "ja");
    expect(ja.problem).not.toBe(deck.problem);
  });
  ...
  it("refines AUTH_FAILED into no-context / api-key / custom-route subtypes", () => {
    ...
    const customDeck = buildErrorDeck("AUTH_CUSTOM_PROVIDER", "en");
    expect(customDeck.primary.id).toBe("open_providers");
    expect(customDeck.cause.toLowerCase()).toMatch(/provider|key|relay/);
  });
