...
describe("sidePickerOptions", () => {
  ...
  it("Office picker keeps files and browser only", () => {
    const opts = sidePickerOptions({
      isGitProject: true,
      workMode: "office",
    });
    expect(opts.map((o) => o.kind)).toEqual(["file", "browser"]);
    expect(
      isPickerCreatableKind("terminal", {
        isGitProject: true,
        workMode: "office",
      }),
    ).toBe(false);
    expect(
      isPickerCreatableKind("review", {
        isGitProject: true,
        workMode: "office",
      }),
    ).toBe(false);
  ...
  it("prunes coding side tabs in Office", () => {
    let s = emptySideWorkbenchState();
    s = openSideTab(s, "file", { path: "docs/q3.docx", name: "q3.docx" });
