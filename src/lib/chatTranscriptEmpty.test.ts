...
describe("resolveChatTranscriptEmptyState", () => {
  ...
  it("uses daily-work copy in Grok Office", () => {
    expect(
      resolveChatTranscriptEmptyState({ empty: true, officeMode: true }),
    ).toEqual({
      kind: "start",
  ...
  it("names the Grok Build folder carried into Office", () => {
    expect(
      resolveChatTranscriptEmptyState({
        empty: true,
        officeMode: true,
        officeProjectName: "grok-app",
      }),
    ).toEqual({
      kind: "start",
