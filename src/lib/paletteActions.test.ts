...
describe("filterPaletteActions", () => {
  ...
  it("matches by keyword", () => {
    ...
    const office = filterPaletteActions("cowork spreadsheet", catalog);
    expect(office.map((h) => h.id)).toContain("open-office");

    const report = filterPaletteActions("code report", catalog);
    expect(report.map((h) => h.id)).toContain("office-report");
