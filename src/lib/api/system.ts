...
export async function discordPresenceUpdate(payload: {
  details: string;
  state: string;
  startSec: number;
  clientId?: string;
  ...
  try {
    await invoke<void>("discord_presence_update", {
...
export async function discordPresenceClear() {
  if (!isDesktopHost()) return;
  try {
    await invoke<void>("discord_presence_clear");
  } catch {
    /* Discord optional */
  }
}

export async function discordPresenceProbe(clientId?: string): Promise<{
  ok: boolean;
  error: string | null;
}> {
  if (!isDesktopHost()) {
  ...
  try {
    return await invoke<{ ok: boolean; error: string | null }>(
