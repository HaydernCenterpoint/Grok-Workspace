...
### Changed
- **Grok Studio / Office composer prefs are per surface**: Studio hides Build’s workspace, model, effort, and Full access chips and locks Imagine. Office keeps its own model / mode / policy in `localStorage` and does not write Host `composer_prefs_*`. Switching back to Build restores Host prefs.
- **Grok Studio is a normal chat**: the user prompt, thinking, and Imagine result stay in the transcript above the composer. The bar only has Image / Video controls — no “Generating…” under the box.
- **Boot splash is the Grok mark only**: no welcome title, no “Checking Grok Build…” line, no rounded tile. The mark draws in and sheens while the CLI probe runs.
- **Codex workbench chrome (default)**: solid panes, docked composer, ~240px thread rail, tools pane open on launch. Settings → Appearance → Interface can restore Classic (floating composer + glass). Saved sidebar widths are kept.
- **Chinese UI unshipped**: Settings and OS `zh*` fall back to English. `zh` / `zh-TW` catalogs stay on disk for a later restore. Speech-to-text Chinese options are unchanged.
- **TypeScript 7 for `tsc`**: native compiler 7.0.2 typechecks the app. `typescript-eslint` / Vitest keep the TypeScript 6 API via `@typescript/typescript6` until 7.1 ships a stable programmatic API.
...
**中文 · 变更**
- **Grok Studio / Office 的 composer 偏好按界面分开**：Studio 去掉 Build 的工作区、模型、effort、Full access，锁定 Imagine。Office 自己记 model / mode / policy（`localStorage`），不写 Host `composer_prefs_*`。切回 Build 时还原 Host。
- **Grok Studio 走正常聊天**：用户提示、thinking、Imagine 结果都在输入框上方的对话里。栏上只留 Image / Video，不再在框底下写 Generating…。
- **启动闪屏只留 Grok 标志**：去掉欢迎标题、「正在检测 Grok Build…」和圆角方块。探测 CLI 时标志切入并扫光。
- **默认 Codex 工作台外壳**：实色面板、底部输入框、约 240px 会话轨、启动时打开右侧工具栏。设置 → 外观 → 界面可切回经典（悬浮输入框 + 毛玻璃）。已保存的侧栏宽度不覆盖。
- **暂时下架中文界面**：设置与系统 `zh*` 回落到英文。`zh` / `zh-TW` 目录仍留在仓库，以后再挂回。语音识别中文选项不变。
- **`tsc` 改用 TypeScript 7**：原生编译器 7.0.2 做类型检查。eslint / Vitest 仍走 TypeScript 6 API（`@typescript/typescript6`），等 7.1 稳定 API。
