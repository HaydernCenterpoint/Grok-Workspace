...
- **Each surface holds its own working measure**: Grok Build keeps the code reading width and honors Settings → Appearance → Chat width; Grok Office runs a 52rem document measure at 1.75 line-height; Grok Studio runs 64rem and lets generated media fill the column at its real aspect ratio instead of a 240×150 thumbnail.
...
- **三个界面各有自己的工作尺度**：Grok Build 保持代码阅读宽度并遵循设置 → 外观 → 聊天宽度；Grok Office 用 52rem 文档宽度、1.75 行高；Grok Studio 用 64rem，生成的图按真实比例铺满，不再塞进 240×150 缩略图。
