...
- **Grok-styled Windows installer**: The NSIS setup now ships its own chrome instead of the stock Tauri look. Welcome and Finish run dark (`0A0A0A`) behind a 164×314 sidebar that renders the brushed Grok mark, the `GROK / WORKSPACE` lockup, `BUILD · OFFICE · STUDIO`, and the live version; inner pages keep native Windows chrome with a white 150×57 header carrying the black mark, so no black block fights the header strip. Setup and uninstaller use the app icon, the language picker is gone (English only, one less click), and Apps & features links back to the repo. Art is generated by `scripts/generate-nsis-branding.py` from the two logo sources already in the tree — no binary design files to hand-maintain. Page copy lives in `src-tauri/windows/nsis-hooks.nsh`; Tauri's installer template is untouched.
- **Drag sessions inside the three workspaces**: Drop a chat on a folder on the same workspace to rebind it. Dropping onto another workspace (header or folder) snaps back and does not change home. Recents stays a drop target to unbind a folder chat. In-list order is last activity, not a saved drag rank.

### Changed
- **Unfocused stream IPC waits for 160ms instead of 600-byte bursts**: A fast turn still hit `session://stream` on every ~600 characters while the window was OS-unfocused, so the 160ms coalesce never won. Unfocused char budget is 2400 bytes; focused 40ms / 600-byte flush is unchanged. Phase / `done` still flush immediately. Stream IPC is not skipped.
- **Unfocused mid-stream journal honors the 2s interval on paragraph breaks**: A 60-paragraph climb used to rewrite `messages.json` on every `\n\n` (read + parse + rewrite the whole file) even when the window was OS-unfocused and ConversationThread was not painting. Turn-end / stop still flush immediately; focused paragraph-immediate is unchanged. Stream IPC to the WebView is unchanged.
- **Project actions sit on each workspace**: Select, archive older, collapse folders, and plus moved from the global Projects header onto **Grok Build / Grok Office / Grok Studio** (and Recents when the action applies). Plus on Build or Office opens a solid menu: new chat for that workspace, or Create project already aimed at that workspace. Studio plus starts a studio session. Connect device stays in the nav.
- **Discord Rich Presence always shows plan, quota, model + effort, session, and elapsed**: Details is `{plan} · {remaining%}` (same SuperGrok remaining % as the sidebar; custom route uses the footer balance chip). State is `{model} · {effort} · {session}` (title or id-short). Elapsed is this session while idle and the current turn while working. Hover assets carry session and model · effort. Settings → General → App, still on by default.
- **Wallpaper video parks on real OS window unfocus**: Park follows Tauri `WindowEvent::Focused` (same signal as Host `MAIN_WINDOW_FOCUSED`), not only `document.hidden` / `document.hasFocus()`. WebView can stay “focused” while another app is in front. Hidden, OS-unfocused, or first-run setup **releases wallpaper `<video>` `src` + `load()` on the Host unfocus hook (before React subscribers), then revokes the object URL and drops wallpaper CSS flags**. React unmount waits two frames so WebView can process `load()` (same-turn unmount is the play-level leftover). A remounted wallpaper clip is released again while parked (`data-wallpaper-parked=1`, no `<video>`). WebView drops the decoder (`pause()` or unmount-with-src-set still costs play-level CPU/GPU). The blob stays in memory — refocus remounts the same clip without rewriting shared settings. Clearing a playing clip uses the same unload-then-revoke path. Focused + visible + not streaming-perf plays again. Unfocused-but-visible stream tokens still land in the transcript store; ConversationThread paints on refocus (same flush as a hidden window). Live-turn `data-stream-perf` still pauses wallpaper while you are watching. Still-image wallpaper frost is unchanged.
- **Boot splash and Setup park compositor motion when idle**: The Grok mark sheen/breathe stops on timeout Retry, on Welcome sitting still, and when the window is hidden or unfocused (`document.hidden` is not enough — a visible unfocused window stays `visibilityState=visible`). Wallpaper video under the first-run gate does not decode. The fly-in `will-change` drops after the intro. CLI required / account optional is unchanged.
- **Composer chrome contrast**: The Choose project label, plus and skills (wrench) buttons, and the model pill name are bold `--text-primary`. The model trigger no longer shows a lightning bolt; its pill uses a recessed `--composer-model-pill-bg` darker than the composer surface. Shared dock (Build / Office / Studio).
- **App / dock icon**: Windows `.exe` / installer, macOS `.icns`, and Linux PNGs use the white struck-through mark on the existing black rounded plate. Tray / menu-bar glyphs stay on `docs/svg/logo.svg` (see `docs/llm-wiki/icons.md`).
- **Hidden windows skip the 10-minute SuperGrok quota probe**: The background billing tick parks with the document (`visibleInterval`) so a tray-idle app does not call Host. Showing the window runs one due check. Boot / Account / user-menu / login refreshes are unchanged.
- **Windows Personalize watch is one blocked thread**: App follow-system (`AppsUseLightTheme`) and the tray taskbar badge (`SystemUsesLightTheme`) share a single `RegNotifyChangeKeyValue` waiter and fan out. A flip of one DWORD does not restyle the other. Tray `set_icon` still hops to the main thread (#735).
- **Windows tray theme watch stops polling**: The notification-area badge still follows `SystemUsesLightTheme` (not the in-app theme) and still swaps on the main thread, but the leftover 1s Personalize registry poll is gone. It waits on the same `RegNotifyChangeKeyValue` helper as OS app theme (30s backoff only if notify fails).
- **Windows theme watch and idle Remote IM stop polling**: Windows OS theme uses a registry notify instead of a 1s poll (30s backoff only if notify fails). Remote IM health ticks park while the bridge is disabled (15s only when enabled). Mid-stream `messages.json` durability and hidden-window stream IPC are unchanged.
- **Idle and hidden windows stop waking the machine**: Git dirty, process-budget, Remote IM, session-API, automations, Office doc poll, settings clocks, theme schedule, ghost-heal, and live thinking clocks skip ticks while the document is hidden. Stream tokens still land in the transcript store, but ConversationThread does not re-render until the window is shown. Chat relative timestamps share the sidebar 60s store. Composer git status is one Host spawn (`git status -b`) instead of four, and the live-turn poll is 5s not 2s. Window-resize only commits viewport width once per frame. Host stream-stall / tool-heartbeat walks skip when no session is streaming or mid-tool. Unfocused main window coalesces `session://stream` at 160ms instead of 40ms (char budget / `done` still flush). Pet task-expire ticks are 1s and skip while hidden. Hidden windows disarm shared poll timers instead of no-op ticking. Composer slash/@ detection is event + 2-frame rAF, not a perpetual vsync poll. Hidden windows drop wallpaper/sidebar blur (`data-window-hidden`). Session API persist queue skips spawn and disk when empty. Unfocused mid-turn `messages.json` is 2s not 500ms (turn-end still flushes; paragraph-immediate is focused-only). Idle Host stall watchdog sleeps 30s (5s only with a live turn or session-API queue). Heartbeat reuses the last runtime snapshot (no disk read before the 30s write). Sidebar relative-time 60s timer parks while hidden. Idle recycle skips `connect_lock` when no warm session. Pet-off cursor watch parks 2s and does not query the overlay. Wallpaper video uses `preload=metadata`, skips clip `timeupdate` while paused, and disconnects ResizeObserver while hidden. Empty automation lists skip the 30s disk + session-lock tick after the first load. Empty prewarm slots skip the 30s reap walk. Video wallpaper drops sidebar/settings frost (`data-wallpaper-video`). Composer send gimbal rings park while the window is hidden. Idle git dirty poll is 15s (live turn stays 5s). Fully idle Host heartbeat is 90s. Mid-stream journal writes `messages.json` only; the sessions index `updated_at` waits for turn end (stop / disconnect still flush).
- **Studio empty draft matches Build / Office**: The locked Imagine chip is gone from the composer. A new Studio chat shows the Grok mark and “Show me something” instead of “Imagine an image.” The box placeholder is “Describe what you want.”
- **Studio aspect preview follows the pointer**: Hover or keyboard-focus a ratio and the left frame morphs to that layout. Click still commits the choice.
- **Studio media is a reading card, not a poster**: Generated stills and clips cap at 26rem wide and about 20rem / 40vh tall (portraits shrink width so they stay in that box). Maximize / surface switch reclamps a stale aside strip that used to linger until the window was closed and restored.
- **Studio Image can request up to 4 stills**: A count chip sits after the aspect pill (1–4, default 1). Video keeps resolution / duration instead. Send wraps the agent text so `image_gen` is called once per still.
- **Remote control lists only international IM**: The sidebar keeps Bridge overview plus Telegram, Slack, Discord, Matrix, and LINE. Domestic channels and the Overseas heading are gone from the picker.
- **Sidebar rail is a ChatGPT-soft charcoal**: Dark uses `#171717` with a top wash, no hard divider, and a `#212121` chat canvas so the column sits quietly beside the page. Light uses `#f4f4f4`. Settings nav matches. Wallpaper skins still override the fill.
- **Remote IM reconnects the five remaining channels**: Bridge starts only Telegram / Slack / Discord / Matrix / LINE (leftover domestic instances stay on disk and do not auto-start). IM-created chats are stamped **Grok Build**. Slack Block Kit, Discord components, and LINE Flex handle `/p` `/r` `/account` the same way Telegram inline keyboards do; Matrix keeps numbered text menus.
- **Send Studio media to Build or Office**: Left-click (or right-click) a generated image and choose **Send to Grok Build** or **Send to Grok Office**. That starts a new draft on that surface with the file attached and moves you there. Video has the same rows on its menu.
- **Workspace sessions stay on their surface**: A tagged Studio / Office / Build chat cannot open in another workspace. A leftover Build draft stamp no longer rides into Studio (which listed Imagine chats under Grok Build). Switching surfaces on an unsent draft starts a clean draft for that space.
- **UI type is bundled Inter**: Sidebar, chat, settings, setup, and share/export cards use self-hosted Inter Variable (opsz + wght, including Vietnamese / Latin / Cyrillic) — the same official Inter as the Google Fonts desktop zip. Empty Appearance → UI font keeps that face. Code blocks and the built-in terminal stay monospace. CJK still falls back to the OS face (PingFang / YaHei / Yu Gothic / Malgun).
- **Auto-download is a Settings → About toggle** (on by default, localStorage only). Off still lets background check surface a new version, but does not start the signed download until you use the banner, About, or the sidebar. Signed-only; unsigned builds stay GitHub-manual.
- **Studio Imagine bar matches the reference chips**: Image / Video is one segmented control with a sliding highlight. Aspect opens a named-ratio menu plus a preview frame; video resolution and duration use the same check-list popovers. The Studio composer uses the same chat width as Build.
- **Sidebar tree is three surfaces plus Recents**: Grok Build, Grok Office, and Grok Studio each show project folders and folderless chats tagged for that workspace; `+` starts a session on that surface (not Recents). Opening a project or chat switches to the matching surface. Recents (after Studio) is only for short chats started from Recents `+`. A folder session never also appears there.
- **Folderless chats stay on their surface unless Recents-stamped**: Unstamped folderless chats list under Build / Office / Studio from their work-mode tag (default Build). Recents `+` stamps a chat so it stays in Recents. Drag onto a surface header or folder clears that stamp; drag onto the Recents header unbinds a folder chat and stamps it.
- **Office start is its own canvas, not the last Build chat**: Switching to Grok Office via the sidebar logo, `#/office`, `/office`, or Open Office no longer retags the open Build thread or paints its coding greeting under the Office starters. An empty Office draft shows the Grok mark, “Papers, slides, and sheets”, and the report / slides / sheet rows. Opening a chat from the Office tree still keeps that session.
- **Grok Office is a document canvas**: Word / Excel / PowerPoint IR sits above the composer; type commands below. The right pane loads a Build project or a chosen folder. Surfaces share the first-run theme (no cream paper overlay, no extra chat dividers). The composer dock matches Build — no Grok Office chip (mode is already Office). Canvas edits write `*.office.json`; Save Word / Excel / PowerPoint still compiles. Not a port of OfficeCLI.
- **New-chat stage is Grok logo + slogan + starter lines**: An empty Build draft centers the Grok mark and “Tell me your idea”, then four underlined rows (explore, build, review, fix) with no card frames. The hero shares the composer column (no left shift). The composer, project picker, and Create project dialog sit on a lighter elevated gray so they read against the page. A row seeds the composer; the agent still does the work. Office keeps its own start list. The composer stays at the bottom instead of lifting a SuperGrok wordmark with the input.
- **Project pick and create follow ChatGPT / Codex**: The composer chip says Choose project, the menu searches the list (no folder icons) and ends with New project, and Create project is a name + source-folder dialog. Plus still attaches photos and files.
- **Product switcher menu matches Codex glass**: Build / Office / Studio rows drop the leading icons, use a 16px frosted panel, and keep the check on the selected row.
- **Sidebar account menu is a ChatGPT-style list**: the footer chip shows the profile name; the open menu’s identity row is the subscription plan. Usage (remaining %), Settings, tour, theme, and Sign out follow. The bordered quota card (plan, reset clock, progress bar) is gone — those details stay in Settings → Account and the usage modal.
- **First-run setup names the product and splits sign-in**: Welcome to Grok Workspace, a short creative line instead of the MIT disclaimer, and three standalone account buttons (Grok account, API key, custom provider) instead of one shared card. The Runtime / Account / Ready dots are gone. A successful login or Skip opens a ChatGPT-style appearance step (theme, accent, UI font, chat and code size) before the workbench. The boot mark flies into the header (no tile), then the title and slogan fade in, then the buttons open with a dual horizontal shutter.
- **Composer access matches the ChatGPT approval sheet**: one pill shows the current permission (orange + Grok mark for Full access). The open sheet lists only Ask / Approve for me / Full access. Mode and Advanced are gone from the composer (Plan is a skill). The model pill uses ChatGPT-style Model / Speed / Context window rows.
- **The chat column launches wide**: Codex chrome opened the 400px tools pane on boot, leaving a 1200px window only ~520px of transcript. Launch is now chat-first (~930px at 1200×800) and the pane uncollapses when you open a file, terminal, or review. The reading-width ladder moved up with it, and the measure now lives on the `html` root so the docked composer lines up with the messages instead of sitting 32px narrower.
- **Live activity marks are two loaders on every surface**: thinking and “Working for” use a 3-dot harmonic scale; a running tool uses a 4-square blue pulse. Build, Office, and Studio share the same transcript chrome; a busy sidebar session uses the 3-dot mark. Finished rows keep the bulb / grid / tool icons. Animations sit behind `prefers-reduced-motion`.
- **Folded thinking and tool phases show what happened**: A collapsed thought appends a one-line gist; a collapsed phase summarizes its first three steps in stream order (reasoning first, then tools). Click targets, keyboard behavior, and `aria-expanded` are unchanged — this extends the existing transcript rendering rather than adding a second timeline. Fold wording is shared with the expanded rows via `activityStepLabel` / `activityStepsSummary`, so the two cannot drift.
- **Build / Office / Studio transcripts match the 52rem composer card**: the command column (markdown, thoughts, tool cards, chips) uses `--composer-width-max` on `.lobe-chat__inner`, centered like the docked input. Settings → Chat width no longer widens those workspace columns.
- **Softer workbench chrome**: A two-step elevation ramp (`--elev-1` / `--elev-2`) replaces hand-rolled shadows on bubbles, media and file cards, the docked composer (which lifts on focus), Office artifacts, and Studio media. Codex radii move from 4–6px to 8/10/12 with menus at 10 and modals at 14. The OS window stays square.
- **Micro-interactions on small controls**: Menus and popovers fade in; icon buttons, model and permission chips, menu rows, and Studio pills carry 120ms hover transitions and a press scale. All of it sits behind `prefers-reduced-motion: no-preference`, which also now stops the infinite live-pulse animations that previously ignored the setting.
- **Send button gets counter-rotating gimbal rings**: The shared composer send square (Grok Build / Office / Studio) draws two concentric 3D rings around the still up-arrow. They spin only when send is enabled; hover and focus-visible brighten and slightly speed the rings. `prefers-reduced-motion` keeps them static.

### Fixed
- **Folder chats sort by last activity**: Sessions under a project (and folderless stacks) use the same parsed `updatedAt` as the rest of the app — pinned first, then newest. A leftover `grok.sessionSidebarOrder` drag map no longer freezes “7 hours ago” above “23 minutes ago”. Relative-time labels are display-only (not the comparator).
- **Session home stays on the surface it was created on**: `grok.sessionWorkMode` is a sticky `sessionId → workMode` stamp (first write wins). Creating a chat on Office keeps it under Grok Office after you switch to Build; a Build chat is not listed as Office. Switching surfaces, opening the same folder elsewhere, Recents regroup, or drag onto another workspace cannot retag it. Same-surface folder moves still work.
- **Office landing does not sit on a live transcript**: A session with messages, a **Working** / streaming turn, a journal still loading, or a loaded document unmounts the Grok mark + “Papers, slides, and sheets” rows. Waiting for the first shell message row used to keep the start canvas on a busy Office chat. The command column fills the stage above the docked composer so the last-message toolbar is not pinned against the input. Empty Office still shows the start canvas.
- **Office files pane stays closed after you hide it**: Entering Grok Office still opens Files once. Closing the column, then switching to Grok Build and back, no longer slams it open again. The Files control still opens it on purpose. A live plan no longer steals the Plan tab back after you close it (Open in resources still can).
- **New chat stays on the session you just sent**: The first send used to materialize an id, default it to Grok Build, then open a fresh draft on Studio / Office — so the sidebar never selected the thread you were in. The new id is stamped for that surface before the view swaps, and switching surfaces no longer treats that materialize as a mismatch.
...
- **创建项目时选择工作区**：创建项目对话框增加 **Choose workspace**（Build / Office）。默认是当前工作面（Studio 回退到 Build）。新文件夹的归属存在本地 `grok.projectWorkMode`，侧栏在还没有会话时也会把它列在 Grok Build 或 Grok Office 下。创建后切换到该工作区并选中新项目。
- **从 Codex / Claude 导入 + 查找已安装技能**：设置 → 扩展 → 技能（以及 MCP / 账户 → 提供商）可扫描 `~/.codex`、`~/.claude`、Claude Desktop、`~/.cursor/skills`、`~/.agents/skills`、Grok / 项目技能目录。核对弹窗把勾选的 MCP（只带 command/URL）和 Claude 权限规则写入独立 agent-home，并把 SKILL.md 目录复制到当前技能根。共享模式不会改写 `~/.grok` 的 config.toml，除非你主动切到独立模式。不复制 API 密钥、`auth.json`、`.env`、MCP env/请求头；Codex 提供商行只预览。
- **ChatGPT 式更新条**：签名包下完后，工作台顶部写明版本已就绪，可 **立即重启** 或 **稍后**。下载中只有一句安静提示（没有假进度）。未签名 / 本地 / Linux 非 AppImage 仍是「有新版本」+ **下载**（打开 GitHub）+ **稍后**，绝不声称会静默安装。稍后只藏本会话的条，侧栏图标还在。立即重启仍先走应用内确认，再 `install()` → `prepare_for_app_update` → relaunch。
- **Grok 风格的 Windows 安装包**：NSIS 安装器不再是 Tauri 默认样式。欢迎页与完成页走深色（`0A0A0A`），左侧 164×314 画面是拉丝质感的 Grok 标、`GROK / WORKSPACE` 字标、`BUILD · OFFICE · STUDIO` 与当前版本号；中间几页保留 Windows 原生外观，配 150×57 白底黑标页眉，不会在白色页眉上糊一块黑。安装器与卸载器都用应用图标，去掉语言选择页（仅英文，少点一次），「应用和功能」里可回跳仓库。图片由 `scripts/generate-nsis-branding.py` 从仓库已有的两个 logo 源生成，不需要手工维护设计稿。页面文案在 `src-tauri/windows/nsis-hooks.nsh`；不改 Tauri 的 installer 模板。
- **三个工作面内可拖动会话**：拖到同一工作面的文件夹可改项目绑定。拖到另一个工作面（标题或文件夹）会弹回，不改归属。Recents 仍是拖上去解绑文件夹的目标。组内顺序按最近活动，不再用本地拖动名次盖住时间。

**中文 · 变更**
- **失焦时 stream IPC 按 160ms 合批，不再被 600 字节提前冲掉**：快速回合在窗口失焦时仍几乎每 600 字符就发 `session://stream`，160ms 合并形同虚设。失焦字数预算改为 2400 字节；前台仍是 40ms / 600 字节。阶段切换 / `done` 仍立即刷。不跳过 stream IPC。
- **失焦时中途 journal 不再按段落立即落盘**：60 段爬升以前每遇到 `\n\n` 就整份读写 `messages.json`（读+解析+重写），即使窗口已失焦、ConversationThread 没在画。回合结束 / 停止仍立即写；前台看着时仍按段落立即写。发给 WebView 的 stream IPC 不变。
- **项目操作挪到各工作区标题**：多选、按时间归档、折叠文件夹、加号从全局 Projects 行移到 **Grok Build / Grok Office / Grok Studio**（Recents 只保留适用项）。Build / Office 的加号是实心菜单：在该工作区新建会话，或打开已指向该工作区的创建项目。Studio 加号仍是新建 Studio 会话。Connect device 留在导航。
- **Discord 状态常显套餐、配额、模型 + 推理、会话和已用时长**：第一行是 `{套餐} · {剩余%}`（与侧栏 SuperGrok 剩余% 同源；自定义渠道用页脚余额）。第二行是 `{模型} · {推理} · {会话}`（标题或 id 短号）。空闲按本会话计时，进行中按本回合。悬停资源带会话与模型 · 推理。设置 → 通用 → 应用，默认仍开。
- **壁纸视频在系统窗口失焦时卸下 GPU**：停放跟 Tauri `WindowEvent::Focused`（与 Host `MAIN_WINDOW_FOCUSED` 同一路），不再只看 `document.hidden` / `document.hasFocus()`。WebView 可能仍报 focused，但前台已是别的窗口。隐藏、系统失焦或首次设置门会在 Host 失焦钩子里 **先对壁纸 `<video>` 清 `src` + `load()`（早于 React 订阅者），再 revoke object URL 并摘掉壁纸 CSS 旗标**。React 卸节点等两帧，好让 WebView 处理完 `load()`（同一轮就卸是播放态残留）。停放期间若 React 又挂上壁纸 clip 会再卸一次（`data-wallpaper-parked=1`，DOM 里没有 `<video>`）。合成器才放掉解码器（仅 `pause()` 或带着 src 卸节点仍接近播放态 CPU/GPU）。blob 留在内存，回前台再挂上，不改写共享设置。清掉正在播的 clip 走同一条先卸再 revoke 的路径。前台可见且非 stream-perf 再播。失焦但仍可见时，stream token 仍写入 transcript store，ConversationThread 等到重新聚焦再刷（与隐藏窗口同一套）。你正在看的回合里 `data-stream-perf` 仍只暂停。静图壁纸毛玻璃不变。
- **启动闪屏和首次设置在空闲时停掉合成器动画**：超时重试页、Welcome 静坐、窗口隐藏或失焦时，Grok 标志扫光/呼吸停住（仅 `document.hidden` 不够 —— 仍可见的失焦窗口 `visibilityState` 仍是 `visible`）。门后的壁纸视频不再解码。飞入结束后去掉 `will-change`。CLI 必装、账户可跳过不变。
- **Composer 对比度**：Choose project 标签、加号 / 扳手（技能）按钮、模型药丸名称改为粗体 `--text-primary`。模型触发器去掉闪电标；药丸底用比输入框更深的 `--composer-model-pill-bg`。Build / Office / Studio 共用同一条 dock。
- **应用 / Dock 图标**：Windows `.exe` / 安装包、macOS `.icns` 和 Linux PNG 换成白色斜杠圆标（仍用原来的黑色圆角底板）。托盘 / 菜单栏仍走 `docs/svg/logo.svg`（见 `docs/llm-wiki/icons.md`）。
- **窗口隐藏时不再跑 10 分钟 SuperGrok 额度探测**：后台 billing tick 跟 `visibleInterval` 一样在 `hidden` 时拆掉，托盘挂起不会打 Host。窗口再显示时补一次到期检查。开机 / 账号页 / 用户菜单 / 登录刷新不变。
- **Windows Personalize 监听合成一条线程**：应用跟系统（`AppsUseLightTheme`）和托盘任务栏徽章（`SystemUsesLightTheme`）共用一条 `RegNotifyChangeKeyValue` 等待线程，再分别通知两边。只改其中一个 DWORD 不会带动另一个。托盘 `set_icon` 仍回到主线程（#735）。
- **Windows 托盘主题不再 1 秒轮询**：通知区徽章仍跟任务栏 `SystemUsesLightTheme`（不跟应用内主题），换标仍在主线程，但去掉了每秒读 Personalize 的轮询。与系统应用主题共用 `RegNotifyChangeKeyValue`（notify 失败才 30 秒重试）。
- **Windows 主题监听和空闲 Remote IM 不再空转**：Windows 系统主题改用注册表 notify，不再 1 秒轮询（notify 失败才 30 秒重试）。Remote IM 未启用时 health tick 停住（启用后仍 15 秒一拍）。中途 `messages.json` 耐久和隐藏窗口的 stream IPC 未改。
- **窗口藏到后台就不再空转**：Git 脏状态、进程池、Remote IM、session API、定时任务、Office 文档轮询、设置时钟、主题日程、ghost-heal、思考计时在 `hidden` 时停 tick。Stream token 仍写入 transcript store，但要窗口再显示才重绘 ConversationThread。聊天相对时间跟侧栏共用 60s store。工作区 `git status` 一次进程（带 `-b`）；对话进行中 5 秒一问。窗口缩放每帧只写一次宽度。没有流式/工具会话时 Host 不再走 stall / tool-heartbeat。主窗口失焦时 `session://stream` 合并成 160ms（满字/`done` 仍立即刷）。宠物 task-expire 1 秒一跳，窗口隐藏时停。隐藏时拆掉共享 poll 的 interval，不再空转 wakeup。Composer `/` `@` 改为事件 + 两帧 rAF，不再 60Hz 常驻。窗口隐藏时去掉壁纸/侧栏 backdrop-filter（`data-window-hidden`）。Session API persist queue 为空时 Host 不再 spawn、也不读盘。主窗口失焦时中途 `messages.json` 从 500ms 改 2s 一刷（回合结束仍立即写；按段落立即写只在前台）。空闲时 Host stall watchdog 睡 30s（有回合或 session-API queue 才 5s）。心跳复用上次 runtime 快照，30s 一拍不再先读盘。侧栏相对时间 60s timer 隐藏时拆掉。没有 warm session 时 idle recycle 不抢 connect_lock。宠物关着时 cursor watch 停 2 秒且不查 overlay 窗口。壁纸视频 `preload=metadata`，暂停时不跑 clip `timeupdate`，窗口隐藏时拆 ResizeObserver。没有启用定时任务时 30s tick 不再读盘、也不抢会话锁。prewarm 槽为空时 30s idle watchdog 不再走 reap。视频壁纸去掉侧栏/设置毛玻璃（`data-wallpaper-video`）。窗口隐藏时发送按钮万向环停转。空闲 git dirty 15 秒一问（回合中仍 5 秒）。完全空闲时 Host 心跳 90 秒一拍。中途 journal 只写 `messages.json`；会话索引的 `updated_at` 等到回合结束再写（停止 / 断开仍立即刷）。
- **Studio 空草稿跟 Build / Office 一样**：输入框去掉锁死的 Imagine 芯片。新 Studio 对话居中放 Grok 标和 “Show me something”，不再写 “Imagine an image.”。占位改成 “Describe what you want.”
- **Studio 比例预览跟鼠标走**：悬停或键盘聚焦某一行，左侧画框就变成那个比例。点选才真正改设定。
- **Studio 媒体改成阅读卡片**：生成的图和视频最宽 26rem、最高大约 20rem / 40vh（竖图会收窄以免撑满一栏）。最大化或换工作面会重算侧栏，修掉以前要关掉窗口再最大化才消失的空白条。
- **Studio 出图可一次要最多 4 张**：比例药丸后面加数量芯片（1–4，默认 1）。视频仍用分辨率 / 时长。发送时包装 agent 文本，让 `image_gen` 每张图各调一次。
- **远程控制只列国际 IM**：侧栏只留 Bridge 总览和 Telegram / Slack / Discord / Matrix / LINE。国内渠道和 Overseas 分组标题从选择器里去掉。
- **侧栏底改成 ChatGPT 那种软炭灰**：暗色 `#171717`，顶部一层淡光、没有硬分割线，聊天区用 `#212121`，侧栏才会安静地贴在页面旁边。亮色 `#f4f4f4`。设置页导航同步。壁纸皮肤仍覆盖底色。
- **远程 IM 重接剩余五条渠道**：Bridge 只启动 Telegram / Slack / Discord / Matrix / LINE（国内遗留实例留在磁盘、不自动拉起）。IM 写入的会话打 **Grok Build** 标签。Slack Block Kit、Discord 按钮、LINE Flex 与 Telegram 一样处理 `/p` `/r` `/account`；Matrix 仍用编号文本菜单。
- **Studio 媒体可送到 Build / Office**：左键（或右键）生成图，选 **Send to Grok Build** 或 **Send to Grok Office**，会在对应空间开新草稿、附上该文件并切过去。视频菜单同样有这两项。
- **工作面会话各管各的**：已标记的 Studio / Office / Build 聊天不会在另一个空间打开。Build 草稿上留下的 pending 戳不再跟着进 Studio（Imagine 对话被列到 Grok Build 下面）。在未发送的草稿上换空间会开一份该空间的新草稿。
- **界面字体改为内置 Inter**：侧栏、聊天、设置、首次外观步和分享卡都用自托管 Inter Variable（opsz + wght，含越南语 / 拉丁 / 西里尔），与 Google Fonts 桌面 zip 同一套官方 Inter。外观里 UI 字体留空即这套。代码块和内置终端仍是等宽。中日韩仍回落到系统字体（苹方 / 雅黑 / 游ゴシック / 맑은 고딕）。
- **自动下载是设置 → 关于里的开关**（默认开，只写 localStorage）。关掉后后台检查仍会提示有新版本，但不会自动开始签名下载，需从横幅、关于或侧栏动手。仅签名通道；未签名构建仍走 GitHub 手动。
- **Studio 的 Imagine 栏按参考图重做**：Image / Video 是一组分段开关，高亮会滑过去。比例打开带名称的列表和预览框；视频的分辨率和时长用同一套带勾的弹出菜单。Studio 输入框宽度与 Build 相同。
- **侧栏是三个工作面加 Recents**：Grok Build / Grok Office / Grok Studio 列出项目文件夹以及标记到该空间的无文件夹聊天；`+` 在对应空间开新会话（不会丢进 Recents）。打开项目或会话会切到那个空间。Studio 下面的 **Recents** 只收 Recents `+` 开的短聊。文件夹会话不会同时出现在 Recents。
- **没有文件夹的会话默认留在工作面**：未打 Recents 戳的无文件夹聊天按 work-mode（缺省 Build）列在 Build / Office / Studio。Recents 的 `+` 会打戳，聊天才留在 Recents。拖到工作面标题或文件夹会清戳；拖到 Recents 标题会从文件夹解绑并打戳。
- **Office 起始页不再叠着 Build 对话**：侧栏 logo、`#/office`、`/office` 或「打开 Office」进入时，不再把当前 Build 会话改标成 Office，也不再把写代码的问候画在起步行下面。空的 Office 草稿居中放 Grok 标、「文档、幻灯片和表格」，以及报告 / 幻灯片 / 表格三行。从 Office 树打开的会话仍保留。
- **Grok Office 改成文档画布**：上方直接改 Word / Excel / PowerPoint IR，下方打命令。右侧加载 Build 项目或自选文件夹。三个界面共用首次选择的主题（去掉奶油纸色和多余分隔线）。输入框与 Build 相同，不再放 Grok Office 芯片（侧栏已经是 Office）。画布写回 `*.office.json`；Save Word / Excel / PowerPoint 仍负责编译。不是移植 OfficeCLI。
- **新对话中间是 Grok 标、口号和四行起步**：Build 空草稿居中放 Grok 标和 “Tell me your idea”，下面四行带下划线（探索 / 做功能 / 审改动 / 修故障），没有卡片框。英雄区和输入框同一列宽，不再往左偏。输入框、选项目菜单、Create project 弹窗用更亮的灰面，才压得住底色。点一行只是把草稿填进输入框，活仍由 agent 干。Office 仍用自己的起步列表。输入框留在底部，不再把 SuperGrok 字标抬到输入框上方。
- **选项目 / 建项目按 ChatGPT、Codex**：芯片写 Choose project，菜单可搜索、无文件夹图标、底部 New project；Create project 是名称 + 源文件夹对话框。加号仍用来附加照片和文件。
- **产品切换菜单做成 Codex 那种毛玻璃**：Build / Office / Studio 去掉左侧图标，面板 16px 圆角 + 模糊，选中项只留右侧对勾。
- **侧栏账号菜单改成 ChatGPT 那种一行列表**：底部芯片写资料名；打开后身份行才写订阅套餐。用量（剩余 %）、设置、导览、主题、退出跟在后面。带边框的额度卡（套餐、重置时间、进度条）去掉，细节仍在设置 → 账号和用量弹窗。
- **首次设置写出产品名并拆开登录**：标题改为 Welcome to Grok Workspace，MIT 免责换成一句短创意文案；账号步是三颗独立按钮（Grok 账号 / API Key / 自定义服务商），不再塞进同一张卡片。去掉 Runtime / Account / Ready 三点；登录成功或 Skip 先进入外观步（主题 / 强调色 / UI 字体 / 聊天与代码字号），再进工作台。开机 Grok 标飞到标题位（不再用圆角 tile），然后标题与 slogan 淡入，三颗按钮用左右快门拉开。
- **输入区权限做成 ChatGPT 那种审批菜单**：一颗 pill 显示当前权限（完全访问为橙色 + Grok 标）。打开后只有 Ask / Approve for me / Full access，不再露出 Mode / Advanced（Plan 走 skill）。模型芯片按 ChatGPT 做成 Model / Speed / Context window。
- **聊天列启动即宽**：Codex 外壳开机就开 400px 工具栏，1200px 窗口只剩约 520px 对话区。现在启动以聊天为主（1200×800 下约 930px），打开文件 / 终端 / review 时工具栏才展开。阅读宽度阶梯同步上调，并挂到 `html` 根节点，底部输入框与消息对齐，不再窄 32px。
- **三个界面的进行中活动用同一套指示**：思考和「工作中」是三颗点按节拍放大；正在跑的 tool 是四格蓝点脉冲。Build / Office / Studio 共用对话区 chrome；侧栏会话忙碌也用三颗点。结束后仍用灯泡 / 点阵 / 工具图标。动画收在 `prefers-reduced-motion` 下。
- **折叠的 thinking 与工具阶段会说明做了什么**：折叠的思考补一行要点；折叠的阶段按流顺序摘出前三步（先推理后工具）。点击区域、键盘行为与 `aria-expanded` 不变——这是扩展既有对话渲染，不是加第二套时间线。折叠与展开共用 `activityStepLabel` / `activityStepsSummary`，措辞不会走样。
- **Build / Office / Studio 对话列跟 52rem 输入框对齐**：命令列（正文、思考、工具卡、芯片）读 `--composer-width-max`，落在 `.lobe-chat__inner` 上并居中，和底部输入框同一宽度。设置 → 聊天宽度不再把这三个工作面撑满。
- **更柔和的工作台外壳**：两级阴影（`--elev-1` / `--elev-2`）取代各处手写阴影，用于气泡、图片与文件卡、底部输入框（聚焦时抬起）、Office 文档件与 Studio 媒体。Codex 圆角从 4–6px 提到 8/10/12，菜单 10、弹窗 14。系统窗口仍是直角。
- **小控件的微交互**：菜单与浮层淡入；图标按钮、模型与权限芯片、菜单行、Studio 药丸加 120ms 悬停过渡与按压缩放。全部收在 `prefers-reduced-motion: no-preference` 下，同时修掉此前忽略该设置的无限脉冲动画。
- **发送按钮加上反向旋转的万向环**：Build / Office / Studio 共用的发送方块在静止上箭头外画两圈 3D 环，仅在可发送时慢转；悬停与键盘聚焦更亮、稍快。`prefers-reduced-motion` 下环静止。

**中文 · 修复**
- **文件夹内会话按最近活动排序**：同一项目（以及无文件夹栈）用应用其余部分同一套解析后的 `updatedAt`：置顶在前，然后最新。本地 `grok.sessionSidebarOrder` 拖动名次不再把「7 小时前」压在「23 分钟前」上面。相对时间文案只用于展示，不参与比较。
- **会话留在创建时的工作区**：`grok.sessionWorkMode` 是粘性的 `sessionId → workMode`（先写生效）。在 Office 创建的聊天切到 Build 后仍列在 Grok Office；Build 聊天不会出现在 Office。切换工作面、在别处打开同一文件夹、Recents 重分组、或拖到另一工作面都不能改标。同一工作面内的文件夹拖动仍可用。
- **Office 起始页不再压在已有对话上**：会话里已有消息、**正在 Working / 流式输出**、日记还在加载、或已打开文档时卸掉 Grok 标和「文档、幻灯片和表格」起步行。以前只等 shell 消息数组出现第一行，忙着的 Office 会话会一直停在起始画布。命令列铺满画布上方、停在底部输入框之上，最后一条的工具条不再贴住输入框。空的 Office 草稿仍显示起始页。
- **关掉 Office 文件栏后不会再被拉开**：进 Grok Office 仍会打开一次 Files。关掉后再切到 Grok Build 再回来，不会又把栏弹开。点 Files 仍然会开。计划还在时关掉 Plan 标签也不会被抢回来（「在资源中打开」仍可以）。
- **新对话发送后会停在该会话**：第一次发送会生成 id，却先被当成 Grok Build，再在 Studio / Office 上新开草稿，侧栏就不会选中你正在聊的那条。现在会先按当前工作面打标，再切视图；换工作面也不会把这次落地当成不匹配。
